...
class UnityEnvironment(BaseUnityEnvironment):
	...
	def __init__(self,
		...
		self.task_goal, self.goal_spec = {i: {} for i in range(num_agents)}, {i: {} for i in range(num_agents)}
		...
		for x in satisfied.keys():
			for t in satisfied[x]:
				...
				if str(id_to_grab) in t:
					...
					logger.info(f'{subgoal} is satisfied')
					return True
	...
	def reset(self, environment_graph=None, task_id=None, reset_seed=None):
		# Make sure that characters are out of graph, and ids are ok
		# ipdb.set_trace()
		if reset_seed is not None:
			self.seed = reset_seed
			self.rnd = random.Random(reset_seed)
			logger.info(f"Resetting seed to {self.seed}")
		...
		#TODO: multi-agent goals are not supported yet, so just copy the first one
		# self.task_goal = env_task['task_goal']
		self.task_goal = {i: env_task['task_goal'][0] for i in range(self.num_agents)}
		# print('task_goal: ', self.task_goal)
		logger.info(f'task_goal: {env_task["task_goal"][0]}')
		...
		logger.info(f"Resetting... Envid: {self.env_id}. Taskid: {self.task_id}. Index: {task_id}")

		# TODO: in the future we may want different goals
		# print(self.goal_spec)
		# print(self.task_goal)
		# print(self.agent_goals)
		...
		s, self.cam_count = self.comm.camera_count()
		...
		if environment_graph is not None:
			...
			success, m = self.comm.expand_scene(updated_graph, random_seed=self.seed)
		else:
		...
		# 	rooms = self.rnd.sample(['kitchen', 'bedroom', 'livingroom', 'bathroom'], self.num_agents)
		# else:
		...
		room_info = [(node['class_name'], node['bounding_box']['center']) for node in self.get_graph()['nodes'] if node['class_name'] in ['kitchen', 'bedroom', 'livingroom', 'bathroom']]
		print(room_info)
		for i in range(self.num_agents):
			idx = self.rnd.randint(0, len(room_info)-1)
			position = room_info[idx][1]
			logger.info(f"Adding character{i} at {room_info[idx][0]} {position}")
			if i in self.agent_info:
				self.comm.add_character(self.agent_info[i], position=position) #initial_room=rooms[i]
			else:
				self.comm.add_character(position=position)
		logger.info('------------------')
	...
	def step(self, action_dict):
		...
		logger.info(f"Step {self.steps}, Executing script: {script_list_verbose} \n------------------")
		# print(script_list)
		# print("graph: ", self.get_graph())
		if len(script_list[0]) > 0:
			...
			if self.recording_options['recording']:
				...
				all_script = script_list[0]
				# if self.steps == 0:
				# 	all_script = '<char0> [Walk] <bathroomcabinet> (190)'
				# elif self.steps == 1:
				# 	all_script = '<char0> [Open] <bathroomcabinet> (190)'
				# print(all_script)
				for i in range(3):
					try:
						success, message = self.comm.render_script([all_script],
						recording=True,
						# gen_vid=False,
						skip_animation=False,
						file_name_prefix=self.recording_options['file_name_prefix'],
						output_folder=self.recording_options['output_folder'],
						camera_mode=['PERSON_FROM_BACK', self.cam_count-1],
						...
						if not success:
							...
							logger.info(f"NO SUCCESS: {message}, {script_list}")
							# print(self.get_graph())
				...
				# for j in range(len(individual_script)):
				# 	for i in range(3):
				# 		try:
				# 			success, message = self.comm.render_script([individual_script[j]],
				# 			recording=True,
				# 			# gen_vid=False,
				# 			skip_animation=False,
				...
				# 		except:
				# 			print("Error in rendering, retrying")
				# 			# logger.info("Error in rendering, retrying")
				# 			continue
				...
				# 	for i in range(3):
				# 		try:
				# 			print(f"adding process: {individual_script[j]}")
				# 			# for i in range(3):
				# 			# 	try:
				# 			p = Process(target=self.comm.render_script, kwargs={'script': [individual_script[j]],
				# 				'recording': True,
				# 				# gen_vid: False,
				# 				'skip_animation': False,
				# 				#    camera_mode=self.recording_options['cameras'],
...
#		obs = self.get_observations()
	...
	def get_observation(self, agent_id, obs_type):
		...
		if obs_type == 'partial':
			...
			return {'messages': self.message_said,**self.full_graph, 'location': agent_location}
